...
- **RootEncoder API usage is unverified.** This sandbox has no Android SDK and no network access to Google's Maven repository, so `feature/streaming/domain/StreamingEngine.kt` (the `RtmpCamera2` wrapper) has never been compiled. The method names, parameter order, and `ConnectChecker` callback signatures are written from best recollection of RootEncoder 2.5.3's public API. **Open this file in Android Studio first** and fix any compile errors against the library's actual API before testing live streaming.
