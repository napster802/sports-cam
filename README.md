...
- **RootEncoder API usage (including the scoreboard burn-in) has been read-verified against the library's real 2.5.3 source, but never compiled.** This sandbox has no Android SDK and no network access to Google's Maven repository, so none of `feature/streaming/domain/` has ever been built. `StreamingEngine.kt`'s method names, parameter order, and `ConnectChecker`/`BitrateChecker` callback signatures, plus `ScoreboardOverlayRenderer.kt`'s use of `SurfaceFilterRender`/`GlInterface` (see below), were manually checked line-by-line against `pedroSG94/RootEncoder`'s `2.5.3` tag — including the actual `object_fragment.glsl` shader, to confirm the per-pixel alpha blending the overlay relies on. One fix was applied: catching the checked `IOException`/`CameraOpenException` that `startRecord`/`switchCamera` declare, which Kotlin doesn't force you to handle. **Open this project in Android Studio first** to confirm with a real compile before testing live streaming — see `docs/ARCHITECTURE.md` for the full verification notes.
- **The scoreboard overlay is now burned into the outgoing RTMP stream and recording**, not just the on-screen preview — `ScoreboardOverlayRenderer` draws the scoreboard onto a `SurfaceFilterRender` added to RootEncoder's `GlInterface`, which composites it directly into the encoded frame (verified against RootEncoder's real shader source, see `docs/ARCHITECTURE.md`). The visual styling (fonts, exact spacing) is a from-scratch Canvas re-implementation of the Compose `ScoreboardOverlay` design rather than a pixel-perfect copy, and has not been seen rendered on a real device yet — treat the first on-device run as a design/layout review, not just a functionality check.
...
- **CI** (`.github/workflows/ci.yml`) runs `detekt`, `lintFreeDebug`, and `testFreeDebugUnitTest` on every push/PR — this will be the first time any of this code is actually compiled by a real toolchain. A ProGuard rule audit and a verified signed release build are still outstanding — see [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md).
