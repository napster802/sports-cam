...
4. **Stand up CI.** Done: `.github/workflows/ci.yml` runs `detekt`, `lintFreeDebug`, and `testFreeDebugUnitTest` on every push/PR (JDK 17, `gradle/actions/setup-gradle`), uploading the reports as artifacts. `detekt` is configured with `ignoreFailures = true` for now (`buildUponDefaultConfig = true`, no custom ruleset yet) since this is the first static-analysis pass this codebase has ever had and the findings haven't been triaged — see the comment in `app/build.gradle.kts`. This is also the first time any of this code will be compiled by something other than a human reading it, so expect the first CI run to surface real build errors (most likely in `StreamingEngine.kt`, per priority #1 above).
