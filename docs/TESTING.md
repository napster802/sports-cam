...
- **`StreamingEngine`** — not unit tested at all. It's a thin wrapper around a third-party camera/encoder library; the highest-value test here is a manual, on-device smoke test against a real RTMP destination once the API mismatches (if any) are resolved, not a unit test mocking RootEncoder's internals.
